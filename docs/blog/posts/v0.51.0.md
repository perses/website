...
As explained before, we moved the default plugins to a new repository. Of course, it means that the frontend code has
been moved to this repository. But it also means that the go SDK and CUE SDK corresponding to each plugins have been
...
longer have the plugins code from perses/perses repository.
