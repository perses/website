...
Panels introduces a new feature called `Quick Query Viewer` using which users can quickly check the executed quires without opening the edit panel. 
The feature has added a new, default icon button into the panel header. When clicked a model is opened in which you can see relevant query specs.
